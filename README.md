...
- Directory structure:
    ...
    - `assets` has the WAV samples, PNG images, and TTF fonts
...
- Using CMake instead of Projucer
    ...
    - `juce_add_binary_data` has the sound files, images, and fonts
    ...
    - A few notable things inside the CMakeLists.txt file:
        ...
        - The `juce_add_binary_data()` section in the CMake file adds the WAV, PNG, and TTF files to the plug-in.
...
- Images and fonts:
    - The background image and the Inter Regular / Inter Medium fonts are added as binary data the same way as the WAV samples. The plug-in stays self-contained — no external file dependencies at runtime.
    - All artwork is rendered at 2x, so the background image is 1376×1032 and the plug-in is 688×516. The 2x assets keep everything sharp on Retina / HiDPI displays.
    - Inter is licensed under the SIL Open Font License, so the TTF files can ship inside the plug-in royalty-free.
